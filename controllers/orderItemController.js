...
const selectAllOrderItem = async(req, res) =>{
    let filter = {};
    ...
    try {
        ...
        if(res.status(200) && orderItems){
            res.status(200).json({data: orderItems}, null, 2)
...
const addOrderItem = async(req, res) => {
    ...
    try {
        ...
        const schema = {
            productId: {type: "number", integer: true, positive: true},
            quantity: {type: "number", integer: true, positive: true},
            orderId: {type: "number", integer: true, positive: true},
        ...
        const product = await OrderItem.create(data)
        if (res.status(201) && product) {
            ...
            res.status(404).json({
                message: "Error Occured!!! Order Could not be added to Cart"
            })
        }
        
    } catch (error) {
        res.status(500).json({
            message: "Order Could not be added to Cart - Something went wrong",
            error: error.message
            })
    }
}
